...
public class RecordingTests
{
    ...
    {
        var bitmap = new ConsoleBitmap(4, 2);
        ConsoleBitmap redBitmap = null, greenBitmap = null, magentaPixelBitmap = null;
    ...
    {
        var bitmap = new ConsoleBitmap(1, 1);
        var numFrames = 10000;
        using (var sharedStream = new MemoryStream())
        {
            var bitmapVideoWriter = new ConsoleBitmapVideoWriter(s => sharedStream.Write(Encoding.Default.GetBytes(s)));
            ...
            while ((lastFrameIndex = video.Seek(destination, out frame, lastFrameIndex >= 0 ? lastFrameIndex : 0)) !=
                   numFrames - 1)
            {
    ...
    {
        var app = new CliTestHarness(TestContext, 80, 30);
        ...
        app.InvokeNextCycle(
            async () => {
                ...
                player.SubscribeForLifetime(app, nameof(player.State),
                    () => {
