...
public class ListGridTests
{
    ...
    public void TestDataGridBasic()
    {
        ...
        var dataGrid = new ListGrid<Item>(
            new ListGridOptions<Item> {
        ...
        app.InvokeNextCycle(
            async () => {
                var selectionLabel = app.LayoutRoot.Add(new Label { Text = "DEFAULT".ToConsoleString(), Height = 1 })
                    .CenterHorizontally();

                selectionLabel.Text =
                    $"SelectedRowIndex: {dataGrid.SelectedRowIndex}, SelectedCellIndex: {dataGrid.SelectedColumnIndex}"
                        .ToConsoleString();
    ...
    public class SlowList<T> : CachedRemoteList<T> where T : class
    {
        ...
        public SlowList(List<T> items) { this.items = items; }
