...
public class HitDetectionBenchmark : UIBenchmark
{
    protected override float RunActual(ConsoleApp app)
    {
        ...
        app.Invoke(async () =>
        {
            SpaceTimePanel? stp = app.LayoutRoot.Add(new SpaceTimePanel(new SpaceTime(app.LayoutRoot.Width, app.LayoutRoot.Height))).CenterBoth();
            app.LayoutRoot.Background = RGB.Black;
            stp.Background = new RGB(20, 20, 20);
            stp.SpaceTime.Start();
            var random = new Random(100);
            Label? nLabel = app.LayoutRoot.Add(new Label() { Text = $"N = {n}".ToCyan() }).CenterHorizontally().DockToTop(padding: 2);
            RealTimeState latestState = RealTimeState.Cold;
            stp.SpaceTime.Invoke(async () =>
            {
                stp.RealTimeViewing.RealTimeStateChanged.SubscribeForLifetime(stp.SpaceTime,
                    state =>
                    {
                        latestState = state;
