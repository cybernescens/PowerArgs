...
{
    private readonly Stack<HandlerContext> handlerStack;
    ...
    {
        var alt = keyInfo.Modifiers.HasFlag(ConsoleModifiers.Alt);
        var control = keyInfo.Modifiers.HasFlag(ConsoleModifiers.Control);
        var shift = keyInfo.Modifiers.HasFlag(ConsoleModifiers.Shift);
        var noModifier = !alt && !shift && !control;
        var handlerCount = 0;
    ...
    internal ILifetime PushUnmanaged(ConsoleKey key, ConsoleModifiers? modifier, Action<ConsoleKeyInfo> handler)
    {
        ...
        if (!target.TryGetValue(key, out var targetStack))
        {
    ...
    {
        return PushUnmanaged(key, modifier, _ => { handler(); });
    }
    ...
    {
        PushForLifetime(key, modifier, _ => { handler(); }, manager);
    }

    private class HandlerContext
    {
        internal Dictionary<ConsoleKey, Stack<Action<ConsoleKeyInfo>>> NakedHandlers { get; } = new();
        internal Dictionary<ConsoleKey, Stack<Action<ConsoleKeyInfo>>> AltHandlers { get; } = new();
        internal Dictionary<ConsoleKey, Stack<Action<ConsoleKeyInfo>>> ShiftHandlers { get; } = new();
        internal Dictionary<ConsoleKey, Stack<Action<ConsoleKeyInfo>>> ControlHandlers { get; } = new();
    }
}