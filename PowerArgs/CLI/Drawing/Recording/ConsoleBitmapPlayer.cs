...
public class ConsoleBitmapPlayer : ConsolePanel
{
    ...
    /// </summary>
    private readonly BorderPanel? pictureFrame;

    /// <summary>
    ///     The control that renders the current frame in the video
    /// </summary>
    private readonly BitmapControl? pictureInTheFrame;
    ...
    /// </summary>
    private readonly PlayerProgressBar? playerProgressBar;
    ...
    ///     The buttons that appear under the player progress bar
    /// </summary>
    private readonly Button? seekBack10SButton;

    /// <summary>
    ///     The buttons that appear under the player progress bar
    /// </summary>
    private readonly Button? seekForward10SButton;

    /// <summary>
    ///     The buttons that appear under the player progress bar
    /// </summary>
    private readonly Button? seekToBeginningButton;

    /// <summary>
    ///     The buttons that appear under the player progress bar
    /// </summary>
    private readonly Button? seekToEndButton;
    ...
    public ConsoleBitmapPlayer(bool showButtonBar = true)
    {
        ...
        playerProgressBar = Add(new PlayerProgressBar { ShowPlayCursor = false })
            .FillHorizontally(padding: new Thickness(0, 0, 0, 0)).DockToBottom(padding: 1);

        var buttonBar = Add(new StackPanel { CanFocus = false, Height = 1, Orientation = Orientation.Horizontal })
            .FillHorizontally().DockToBottom();

        seekToBeginningButton = buttonBar.Add(
            new Button {
                Text = "<<".ToConsoleString(), Shortcut = new KeyboardShortcut(ConsoleKey.Home), CanFocus = false
            });

        seekBack10SButton = buttonBar.Add(
            new Button { Shortcut = new KeyboardShortcut(ConsoleKey.LeftArrow), CanFocus = false });

        playButton = buttonBar.Add(
            new Button {
                Text = "".ToConsoleString(), Shortcut = new KeyboardShortcut(ConsoleKey.P), CanFocus = false
            });

        seekForward10SButton = buttonBar.Add(
            new Button { Shortcut = new KeyboardShortcut(ConsoleKey.RightArrow), CanFocus = false });

        seekToEndButton = buttonBar.Add(
            new Button {
        ...
        if (showButtonBar)
        {
            seekToBeginningButton.Pressed.SubscribeForLifetime(this, SeekToBeginningButtonPressed);
            seekBack10SButton.Pressed.SubscribeForLifetime(this, Rewind);
            playButton.Pressed.SubscribeForLifetime(this, PlayPressed);
            seekForward10SButton.Pressed.SubscribeForLifetime(this, FastForward);
            seekToEndButton.Pressed.SubscribeForLifetime(this, SeekToEndButtonPressed);
        }
        else
        {
        ...
        SubscribeForLifetime(this, nameof(State), StateChanged);

        SynchronizeForLifetime(
            nameof(RewindAndFastForwardIncrement),
            () => {
    ...
    /// </summary>
    public TimeSpan RewindAndFastForwardIncrement
    {
        get => Get<TimeSpan>();
        set => Set(value);
    }
    ...
    {
        get => pictureInTheFrame.Bitmap;
        set => pictureInTheFrame.Bitmap = value;
    }

    public Event<TimeSpan> OnFramePlayed { get; } = new();
    ...
    private void StateChanged()
    {
        ...
        else if (State == PlayerState.Failed)
        {
            ...
            Dialog.ShowMessage(Application, failedMessage.ToRed());
        }
        else
        {
    ...
    private async Task PlayLoop()
    {
        ...
        using (var playLifetime = GetPropertyValueLifetime(nameof(State)))
        {
            while (playLifetime.IsExpired == false)
            {
                ...
                var videoLocationPercentage = Math.Round(
                    100.0 * newPlayerPosition.TotalSeconds / duration.Value.TotalSeconds,
                    1,
                ...
                if ((lastFrameIndex = inMemoryVideo.Seek(
                        newPlayerPosition,
                        out seekedFrame,
        ...
        Task.Factory.StartNew(
            () => {
                try
                {
                    ...
                    reader.ReadToEnd(
                        videoWithProgressInfo => {
                            inMemoryVideo = inMemoryVideo ?? videoWithProgressInfo;
                            duration = videoWithProgressInfo.Duration;
                            Application.InvokeNextCycle(
                                () => {
                                    if (CurrentFrame == null)
                                    {
                                        CurrentFrame = videoWithProgressInfo.Frames[0].Bitmap;
                ...
                catch (Exception ex)
                {
                    ...
                    #endif
                    Application.InvokeNextCycle(() => { State = PlayerState.Failed; });
                }
            });
    }
}