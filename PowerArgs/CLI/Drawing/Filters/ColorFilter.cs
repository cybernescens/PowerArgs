...
{
    public ColorFilter(in RGB color) { Color = color; }
    ...
    public void Filter(ConsoleBitmap bitmap)
    {
        for (var x = 0; x < bitmap.Width; x++)
        {
            for (var y = 0; y < bitmap.Height; y++)
            {
                ...
                if (pixel.BackgroundColor != pixel.ForegroundColor &&
                    pixel.BackgroundColor == RGB.Black &&
                    pixel.Value != ' ')
                {
            ...
            for (var y = 0; y < bitmap.Height; y++)
            {
                ...
                bitmap.SetPixel(
                    ...
                    new ConsoleCharacter(
                        pixel.Value,
                        pixel.ForegroundColor.ToOther(RGB.Black, Percentage),
        ...
        for (var x = 0; x < bitmap.Width; x++)
        {
            for (var y = 0; y < bitmap.Height; y++)
            {
                ...
                bitmap.SetPixel(
                    ...
                    new ConsoleCharacter(
                        pixel.Value,
                        RGB.Black.ToOther(pixel.ForegroundColor, Percentage),
...
{
    public static async Task<AnimationFilter> FadeAnimate(
        this ConsoleControl c,
        RGB from,
        RGB to,
        float duration = 500,
        EasingFunction easingFunction = null,
        float fromPerecntage = 0,
    ...
    public static async Task<FadeInFilter> FadeIn(
        this ConsoleControl c,
        float duration = 500,
        EasingFunction easingFunction = null,
        float percentage = 1,
    ...
    public static async Task<FadeOutFilter> FadeOut(
        this ConsoleControl c,
        float duration = 500,
        EasingFunction easingFunction = null,
        float percentage = 1,
