...
public class GridLayoutOptions
{
    ...
    ///     Column definitions
    /// </summary>
    public List<GridColumnDefinition> Columns { get; init; } = new();

    /// <summary>
    ///     Row definitions
    /// </summary>
    public List<GridRowDefinition> Rows { get; init; } = new();
}
...
public class GridRowDefinition : GridValueDefinition
{
    ...
    public static GridRowDefinition Pixels(int pixels) => new() { Height = pixels, Type = GridValueType.Pixels };
    ...
    public static GridRowDefinition Percentage(double percentage) =>
        new() { Height = percentage, Type = GridValueType.Percentage };
    ...
    public static GridRowDefinition Remainder(double remainder) =>
        new() { Height = remainder, Type = GridValueType.RemainderValue };
}
...
public class GridColumnDefinition : GridValueDefinition
{
    ...
    public static GridColumnDefinition Pixels(int pixels) => new() { Width = pixels, Type = GridValueType.Pixels };
    ...
    public static GridColumnDefinition Percentage(double percentage) =>
        new() { Width = percentage, Type = GridValueType.Percentage };
    ...
    public static GridColumnDefinition Remainder(double remainder) =>
        new() { Width = remainder, Type = GridValueType.RemainderValue };
}
...
public class GridLayout : ProtectedConsolePanel
{
    ...
    private readonly List<GridLayoutAssignment> layoutAssignments = new();
    ...
    {
        Options = options;
        SubscribeForLifetime(this, nameof(Bounds), HandleSizeChanged);
        ProtectedPanel.Controls.Removed.SubscribeForLifetime(this, HandleControlRemoved);
    }

    public GridLayoutOptions Options { get; }
    ...
    /// <summary>
    ///     Gets the current row height for the given row index
    /// </summary>
    /// <param name="row">the index of the row to inspect</param>
    /// <returns>the current row height for the given row index</returns>
    ...
    /// <param name="row">the row in which the control will be placed</param>
    ...
    {
        var assignment = layoutAssignments.First(a => a.Control == control);
    ...
    {
        var toRemove = layoutAssignments.FirstOrDefault(a => a.Control == c);

        if (toRemove != null)
            layoutAssignments.Remove(toRemove);
    }

    private void HandleSizeChanged() { RefreshLayout(); }

    public void RefreshLayout()
    {
        columnWidths = ConvertDefinitionsIntoAbsolutePixelSizes(
            Options.Columns.Select(c => c as GridValueDefinition).ToList(),
            Width);

        rowHeights = ConvertDefinitionsIntoAbsolutePixelSizes(
            Options.Rows.Select(c => c as GridValueDefinition).ToList(),
            Height);
    ...
    private RectF GetCellArea(GridLayoutAssignment assignment) =>
        GetCellArea(assignment.Column, assignment.Row, assignment.ColumnSpan, assignment.RowSpan);

    private RectF GetCellArea(int column, int row, int columnSpan = 1, int rowSpan = 1)
    {
        var xOffset = 0;
        for (var x = 0; x < column; x++)
            xOffset += columnWidths[x];
    ...
    private record GridLayoutAssignment
    {
        public ConsoleControl Control { get; init; } = null!;
