...
public class Series
{
    ...
    /// </summary>
    public ConsoleCharacter PlotCharacter { get; set; } = new('X', ConsoleColor.White);
...
public class XYChart : ConsolePanel
{
    ...
    private readonly Label? chartTitleLabel;

    private DataPointControl firstControl;

    private DataPointControl? lastFocused;

    private readonly XYChartOptions options;
    private readonly Label? seriesTitleLabel;
    ...
    public XYChart(XYChartOptions options)
    {
        ...
        AddDataPoints();
        SubscribeForLifetime(this, nameof(Bounds), PositionDataPoints);

        var defaultSeriesTitle = ConsoleString.Empty;

        if (options.Data.First().AllowInteractivity == false || options.Data.Count == 1)
        {
            defaultSeriesTitle = options.Data.First().Title.ToConsoleString(
                options.Data.First().PlotCharacter.ForegroundColor,
                options.Data.First().PlotCharacter.BackgroundColor);
        }

        Ready.SubscribeOnce(
            () => {
                Application.FocusManager.SubscribeForLifetime(this, nameof(FocusManager.FocusedControl),
                    () => {
                        if (options.Data.Count > 1 &&
                            Application.FocusManager.FocusedControl is DataPointControl == false)
                        {
        ...
        chartTitleLabel = Add(new Label { ZIndex = TitleZIndex, Text = options.Title }).CenterHorizontally()
            .DockToTop(padding: 2);

        seriesTitleLabel = Add(new Label { ZIndex = TitleZIndex, Text = defaultSeriesTitle }).CenterHorizontally()
            .DockToTop(padding: 3);
        ...
        Application.FocusManager.SubscribeForLifetime(this, nameof(Application.FocusManager.FocusedControl),
            () => {
    ...
    private int XAxisYValue => Height - XAxisBottomOffset;
    private int XAxisRight => Width - 1;
    private int XAxisLeft { get; set; }
    ...
    private double MinXValueInPlotArea
    {
        get {
            ...
            var trueMin = GetMinValueAcrossSeries(
                options.Data,
                s => s.Points.Count == 0 ? double.MaxValue : s.Points.Select(p => p.X).Min());

            var trueMax = GetMaxValueAcrossSeries(
                options.Data,
                s => s.Points.Count == 0 ? double.MinValue : s.Points.Select(p => p.X).Max());
    ...
    private double MinYValueInPlotArea
    {
        get {
            ...
            var trueMin = GetMinValueAcrossSeries(
                options.Data,
                s => s.Points.Count == 0 ? double.MinValue : s.Points.Select(p => p.Y).Min());

            var trueMax = GetMaxValueAcrossSeries(
                options.Data,
                s => s.Points.Count == 0 ? double.MaxValue : s.Points.Select(p => p.Y).Max());
    ...
    private double MaxXValueInPlotArea
    {
        get {
            ...
            var trueMin = GetMinValueAcrossSeries(
                options.Data,
                s => s.Points.Count == 0 ? double.MinValue : s.Points.Select(p => p.X).Min());

            var trueMax = GetMaxValueAcrossSeries(
                options.Data,
                s => s.Points.Count == 0 ? double.MaxValue : s.Points.Select(p => p.X).Max());
    ...
    private double MaxYValueInPlotArea
    {
        get {
            ...
            var trueMin = GetMinValueAcrossSeries(
                options.Data,
                s => s.Points.Count == 0 ? double.MaxValue : s.Points.Select(p => p.Y).Min());

            var trueMax = GetMaxValueAcrossSeries(
                options.Data,
                s => s.Points.Count == 0 ? double.MinValue : s.Points.Select(p => p.Y).Max());
    ...
    /// </summary>
    /// <param name="data">
    ///     the data points, where each inner array is expected to have 2 values, the first being the X value
    ///     and the second being the Y value
    /// </param>
    ...
    public static void Show(IEnumerable<Tuple<double, double>> data) =>
        Show(data.Select(d => new DataPoint { X = d.Item1, Y = d.Item2 }));

    /// <summary>
    ///     Shows the data in a chart via an interactive console app
    /// </summary>
    /// <param name="data">the data points to plot</param>
    public static void Show(IEnumerable<DataPoint> data) =>
        Show(new XYChartOptions { Data = new List<Series> { new() { Points = data.ToList() } } });
    ...
    public void Refresh()
    {
        chartTitleLabel.Text = options.Title;
        AddDataPoints();
        PositionDataPoints();
    }
    ...
    {
        RenderYAxis(
            context); // MUST render y axis first because the variable y axis label widths cause the plot area to change
    ...
    private void PaintXAxis(ConsoleBitmap? context)
    {
        ...
        context.DrawLine(
            ...
            XAxisYValue,
            XAxisRight,
            XAxisYValue); // horizontal line at the bottom of the x axis
        ...
        if (MinXValueInPlotArea != double.PositiveInfinity && MaxXValueInPlotArea != double.NegativeInfinity)
        {
            cachedXAxisLabels = cachedXAxisLabels ??
                options.XAxisFormatter.GetOptimizedAxisLabelValues(
                    MinXValueInPlotArea,
                    MaxXValueInPlotArea,
                    maxNumberOfLabels).Select(
                    d => {
    ...
    ///     that determine the plot area. It needs to be called anytime the data has changed, and this next part is important,
    ///     it must
    ///     be called as the very first part of the rendering pass since just about all of the rendering depends on the plot
    ///     area being defined.
    /// </summary>
    ...
    private List<AxisLabelInfo> DetermineYAxisLabels()
    {
        ...
        {
            foreach (var labelValue in options.YAxisFormatter.GetOptimizedAxisLabelValues(
                         MinYValueInPlotArea,
                         MaxYValueInPlotArea,
                         maxNumberOfLabels))
            {
                ...
                XAxisLeft = Math.Max(XAxisLeft, label.Length);
            }
        }
    ...
    private void RenderYAxis(ConsoleBitmap? context)
    {
        ...
        context.DrawLine(
            new ConsoleCharacter('|', Foreground, Background),
            XAxisLeft,
            YAxisTop,
            XAxisLeft,
            YAxisBottom);
    ...
    private void AddDataPoints()
    {
        ...
        foreach (var control in Controls.Where(c => c != chartTitleLabel && c != seriesTitleLabel).ToArray())
        ...
        foreach (var series in options.Data)
            ...
            {
                var pixel = new DataPointControl {
                    ZIndex = DataPointsZIndex, CanFocus = series.AllowInteractivity, DataPoint = series.Points[i],
                    Series = series, Value = series.PlotCharacter
                };
                ...
                pixel.Focused.SubscribeForLifetime(
                    ...
                    () => {
                        ...
                        pixel.Value = new ConsoleCharacter(
                            series.PlotCharacter.Value,
                            ConsoleColor.Black,
                            ConsoleColor.Cyan);
                        ...
                        var xValue = options.XAxisFormatter.FormatValue(
                            MinXValueInPlotArea,
                            MaxXValueInPlotArea,
                            pixel.DataPoint.X);

                        var yValue = options.YAxisFormatter.FormatValue(
                            MinYValueInPlotArea,
                            MaxYValueInPlotArea,
                            pixel.DataPoint.Y);
                        ...
                        {
                            newTitle += new ConsoleString(
                                " ( " + xValue + ", " + yValue + " )",
                                series.PlotCharacter.ForegroundColor,
                                series.PlotCharacter.BackgroundColor);
                        }
                        else
                        {
                            newTitle += new ConsoleString(
                                $" ( {xValue},{yValue} - {pixel.DataPoint.Description} )",
                                series.PlotCharacter.ForegroundColor,
                                series.PlotCharacter.BackgroundColor);
                        }
    ...
    private void PositionDataPoints()
    {
        ...
        Controls.WhereAs<BarOrLineControl>().ToList().ForEach(c => Controls.Remove(c));
        var dataPointControlsGroups = Controls.Where(c => c is DataPointControl).Select(c => c as DataPointControl)
            .GroupBy(c => c.Series).ToList();
            ...
            for (var i = 0; i < dataPointControls.Count; i++)
            {
                ...
                else if (control.Series.PlotMode == PlotMode.Lines && i < dataPointControls.Count - 1)
                {
                    ...
                    if (newX2 >= 0 && newX2 < Width && newY2 >= 0 && newY2 < Height)
                    {
                        ...
                        var len = ConsoleBitmap.DefineLineBuffered(control.X, control.Y, nextControl.X, nextControl.Y, out var buffer);
                        for (var j = 0; j < len; j++)
                        {
                            var point = buffer[j];
                            var line = new BarOrLineControl {
                                ...
                                Value = new ConsoleCharacter(
                                    '-',
                                    control.Series.PlotCharacter.ForegroundColor,
                                    control.Series.PlotCharacter.BackgroundColor)
                            };
    ...
    {
        var focusedPoint = Application.FocusManager.FocusedControl as DataPointControl ?? lastFocused;
    ...
    {
        var focusedPoint = Application.FocusManager.FocusedControl as DataPointControl;
        if (focusedPoint == null || Controls.Contains(focusedPoint) == false) return;
    ...
    {
        var xRange = MaxXValueInPlotArea - MinXValueInPlotArea;
        var delta = x - MinXValueInPlotArea;
        var percentage = delta / xRange;
        ...
        var xConverted = XAxisLeft + XAxisWidth * percentage;
    ...
    private double ConvertXPixelToValue(int x)
    {
        ...
        var percentage = delta / XAxisWidth;
        ...
        var dataRange = MaxXValueInPlotArea - MinXValueInPlotArea;
        var xConverted = MinXValueInPlotArea + dataRange * percentage;
        return xConverted;
    }

    private int ConvertYValueToPixel(double y)
    {
        var yRange = MaxYValueInPlotArea - MinYValueInPlotArea;
        var delta = y - MinYValueInPlotArea;
        var percentage = delta / yRange;
        ...
        var yConverted = YAxisBottom - YAxisHeight * percentage;
    ...
    private double ConvertYPixelToValue(int y)
    {
        ...
        var percentage = delta / YAxisHeight;
        ...
        var dataRange = MaxYValueInPlotArea - MinYValueInPlotArea;
        var yConverted = MinYValueInPlotArea + dataRange * percentage;
        return yConverted;
    }
    ...
    private double CalculateDistanceBetween(DataPoint a, DataPoint b) =>
    ...
    private class DataPointControl : PixelControl
    {
        ...
        public List<BarOrLineControl?> BarsOrLines { get; } = new();
    }

    private class BarOrLineControl : PixelControl
    {
        public BarOrLineControl() { CanFocus = false; }
    }
}
...
public class NumberFormatter : IAxisFormatter
{
    ...
    public List<double> GetOptimizedAxisLabelValues(double min, double max, int maxNumberOfLabels)
    {
        ...
        var bestFitLabelsIncrement = incrementBases.OrderBy(
            increment => {
...
{
    private const int MaxLabelsOnAnyChartAssumption = 50;
    ...
    public List<double> GetOptimizedAxisLabelValues(double min, double max, int maxNumberOfLabels)
    {
        ...
        for (var i = 0; i < incrementorCandidates.Count; i++)
        {
            ...
            if (candidate is TimeSpanIncrementor &&
                (candidate as TimeSpanIncrementor).increment.TotalSeconds * MaxLabelsOnAnyChartAssumption <
                xRange.TotalSeconds)
            {
            ...
            else if (candidate is MonthIncrementor &&
                     (candidate as MonthIncrementor).increment * 30 * MaxLabelsOnAnyChartAssumption < xRange.TotalDays)
            {
                incrementorCandidates.RemoveAt(i--);
            }
            else if (candidate is YearIncrementor &&
                     (candidate as YearIncrementor).increment * 365 * MaxLabelsOnAnyChartAssumption < xRange.TotalDays)
            {
        ...
        var bestIncrementor = incrementorCandidates.OrderBy(
            si => {
...
public class TimeSpanFormatter : IAxisFormatter
{
    ...
    public List<double> GetOptimizedAxisLabelValues(double min, double max, int maxNumberOfLabels)
    {
        ...
        if (xRange.TotalDays > 365 * MaxLabelsOnAnyChartAssumption)
        {
            return new NumberFormatter().GetOptimizedAxisLabelValues(
                    minX.TotalDays,
                    maxX.TotalDays + 1,
                    maxNumberOfLabels)
    ...
    public ConsoleString? FormatValue(double min, double max, double value)
    {
        ...
        if (t.Round(TimeSpan.FromDays(1)) == t)
        {
            ...
            if (t.TotalDays == 0 && range < TimeSpan.FromMinutes(1))
            {
                return "0s".ToConsoleString();
            }

            if (t.TotalDays == 0 && range < TimeSpan.FromHours(1))
            {
                return "0m".ToConsoleString();
            }

            if (t.TotalDays == 0 && range < TimeSpan.FromHours(24))
            {
        ...
        if (t.Round(TimeSpan.FromHours(1)) == t)
        {
            ...
            if (t.TotalHours == 0 && range < TimeSpan.FromMinutes(1))
            {
                return "0s".ToConsoleString();
            }

            if (t.TotalHours == 0 && range < TimeSpan.FromHours(1))
            {
                return "0m".ToConsoleString();
            }
        ...
        if (t.Round(TimeSpan.FromMinutes(1)) == t)
        {
            ...
            if (t.TotalMinutes == 0 && range < TimeSpan.FromMinutes(1))
            {
                return "0s".ToConsoleString();
            }
...
internal class TimeSpanIncrementor : TimeIncrementor
{
    ...
    public TimeSpanIncrementor(TimeSpan span) { increment = span; }
    public override DateTime Increment(DateTime original) => original + increment;
    public override DateTime Floor(DateTime original) => new(original.Ticks - original.Ticks % increment.Ticks);

    public TimeSpan Increment(TimeSpan original) => original + increment;

    public TimeSpan Floor(TimeSpan original) => new(original.Ticks - original.Ticks % increment.Ticks);
...
internal class HalfMonthIncrementor : TimeIncrementor
{
    public override DateTime Increment(DateTime original)
    {
        if (original.Day == 15)
        {
            return new DateTime(original.Year, original.Month, 1).AddMonths(1);
        }

        if (original.Day == 1)
        {
            return new DateTime(original.Year, original.Month, 15);
        }

        throw new ArgumentException("Only supports 1st and 15th");
    }
...
internal class MonthIncrementor : TimeIncrementor
{
    ...
    public override DateTime Floor(DateTime original) => new(original.Year, original.Month, 1);
}
...
internal class YearIncrementor : TimeIncrementor
{
    ...
    public override DateTime Floor(DateTime original) => new(original.Year - original.Year % increment, 1, 1);
}