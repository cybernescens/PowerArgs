...
public static class Layout
{
    ...
    {
        var effectivePadding = padding ?? new Thickness(0, 0, 0, 0);

        return DoParentTriggeredLayoutAction(
            child,
            parent,
            (c, p) => {
    ...
    public static T FillAndPreserveAspectRatio<T>(this T child, Container? parent = null, Thickness? padding = null)
    ...
    public static T FillHorizontally<T>(this T child, Container? parent = null, Thickness? padding = null)
        where T : ConsoleControl
    {
        var effectivePadding = padding ?? new Thickness(0, 0, 0, 0);
        ...
        return DoParentTriggeredLayoutAction(
            ...
            (c, p) => {
                ...
                c.Bounds = new RectF(
                    ...
                    c.Y,
                    p.Width - (effectivePadding.Right + effectivePadding.Left),
                    c.Height);
            });
    }

    public static T FillVertically<T>(this T child, Container? parent = null, Thickness? padding = null)
        where T : ConsoleControl
    {
        var effectivePadding = padding ?? new Thickness(0, 0, 0, 0);
        ...
        return DoParentTriggeredLayoutAction(
            child,
            parent,
            (c, p) => c.Bounds = new RectF(
                c.X,
                effectivePadding.Top,
                c.Width,
                p.Height - (effectivePadding.Top + effectivePadding.Bottom)));
    }
    ...
    public static T DockToTop<T>(this T child, Container? parent = null, int padding = 0) where T : ConsoleControl =>
        DoTwoWayLayoutAction(child, parent, (c, _) => c.Y = padding);
    ...
    public static T DockToLeft<T>(this T child, Container? parent = null, int padding = 0) where T : ConsoleControl =>
        DoTwoWayLayoutAction(child, parent, (c, _) => c.X = padding);

    private static T DoTwoWayLayoutAction<T>(this T child, Container? parent, Action<T, Container> action)
        where T : ConsoleControl
    {
        ...
        void SyncAction() => action(child, p);
        child.SubscribeForLifetime(p, nameof(ConsoleControl.Bounds), SyncAction);
        p.SubscribeForLifetime(p, nameof(ConsoleControl.Bounds), SyncAction);
        SyncAction();
        return child;
    }

    private static T DoParentTriggeredLayoutAction<T>(this T child, Container? parent, Action<T, Container> action)
        where T : ConsoleControl
    {
        if (parent == null && child.Parent == null)
            throw new ArgumentNullException(nameof(parent));
        ...
        var p = parent ?? child.Parent;
        void SyncAction() => action(child, p);
        p.SubscribeForLifetime(parent, nameof(ConsoleControl.Bounds), SyncAction);
        SyncAction();
        return child;
    }
}