...
public class Velocity2
{
    ...
    public Event OnAngleChanged { get; } = new();
    public Event OnSpeedChanged { get; } = new();
    public Event BeforeMove { get; } = new();
    public Event OnVelocityEnforced { get; } = new();
    public Event<Impact> ImpactOccurred { get; } = new();
...
public class ColliderGroup
{
    ...
    private const float EvalFrequencySlope =
    ...
    private readonly HitPrediction hitPrediction;
    private TimeSpan lastExecuteTime;
    private readonly ILifetimeManager lt;
    private readonly int maxCapacity;
    private RectF[] obstacleBuffer;

    private VelocityHashTable velocities;

    public ColliderGroup(ConsoleApp application, ILifetimeManager lt, int maxCapacity = 2500)
    {
        this.lt = lt;
        this.maxCapacity = maxCapacity;
        hitPrediction = new HitPrediction();
        application.Invoke(ExecuteAsync);
    }
        ...
        for (var i = 0; i < vSpan.Length; i++)
        {
            ...
            for (var j = 0; j < entry.Length; j++)
            {
                ...
                if (hitPrediction.Type != HitType.None && hitPrediction.LKGD <= d)
                {
                    ...
                    var proposedBounds = velocity.BoundsTransform != null
                        ? velocity.BoundsTransform()
                        : item.Collider.Bounds;

                    var distanceToObstacleHit = proposedBounds.CalculateDistanceTo(obstacleHit.Bounds);
                    if (distanceToObstacleHit > .5f)
                    {
                        proposedBounds = proposedBounds.OffsetByAngleAndDistance(
                            velocity.Angle,
                            distanceToObstacleHit - .5f,
                            false);

                        item.Collider.Bounds = new RectF(
                            proposedBounds.Left - dx,
                            proposedBounds.Top - dy,
                            item.Collider.Width(),
                            item.Collider.Height());
                    ...
                    if (velocity.haveMovedSinceLastHitDetection)
                    {
                        ...
                        if (velocities.TryGetValue(obstacleHit, out var vOther))
                        {
                            if (vOther.Bounce)
                            {
                                var topOrBottomEdgeWasHit = hitPrediction.Edge == obstacleHit.Bounds.TopEdge ||
                                    hitPrediction.Edge == obstacleHit.Bounds.BottomEdge;

                                vOther.angle = topOrBottomEdgeWasHit
                                    ? Angle.Right.Add(-vOther.Angle.Value)
                                    : Angle.Left.Add(-vOther.Angle.Value);
                            }

                            vOther.ImpactOccurred.Fire(
                                new Impact {
                    ...
                    if (velocity.Bounce)
                    {
                        ...
                        velocity.angle = topOrBottomEdgeWasHit
                            ? Angle.Right.Add(-velocity.Angle.Value)
                            : Angle.Left.Add(-velocity.Angle.Value);
                    }
                    else
                    {
                        velocity.Stop();
