...
{
    public Dictionary<TimeSpan, List<string>> Data { get; } = new();

    public void Track(string reason)
    {
        if (Data.TryGetValue(Time.CurrentTime.Now, out var reasons) == false)
        {
...
public class TimeDebuggerPanel : ConsolePanel
{
    ...
    private readonly TimeDebuggingData data;
    ...
    public TimeDebuggerSettings Settings { get; } = new();
    ...
    private void InitLeftPane()
    {
        ...
        Settings.SubscribeForLifetime(this, AnyProperty, () => { settingsHasChanged = true; });
            ...
            new GridLayout(
                new GridLayoutOptions {
                    Columns = new List<GridColumnDefinition> {
                        new() { Width = .25, Type = GridValueType.Percentage },
                        new() { Width = .75, Type = GridValueType.Percentage }
                    },
            ...
            foreach (var str in pair.Value)
                if (str != "Clogged")
                {
                    if (string.IsNullOrWhiteSpace(Settings.SearchFilter) ||
                        str.IndexOf(Settings.SearchFilter, StringComparison.OrdinalIgnoreCase) >= 0)
                    {
                        innerList.Add(str);
                    }
                }
