...
{
    private readonly PerfTestArgs args;
    ...
    private TestOptions GetOptionsForArg() =>
        GetType()
            .GetMethod(args.Test.ToString(), BindingFlags.Instance | BindingFlags.NonPublic)
    ...
    private TestOptions MinimumChanges() => new TestOptions();
    ...
    private TestOptions BouncingBall()
    {
        return new TestOptions {
            InitTest = async () => {
                ...
                await ball.AnimateAsync(
                    new ConsoleControlAnimationOptions {
                        Loop = Forever,
                        Duration = 1000,
    ...
    private async void Init()
    {
        ...
        var messageLabel = messagePanel
            .Add(new Label { Text = "Waiting".ToConsoleString(ConsoleColor.Black, ConsoleColor.Red) }).CenterBoth();

        var now = DateTime.Now;
        var paintsNow = Application.TotalPaints;
        while ((DateTime.Now - now).TotalSeconds < 3)
        {
            messageLabel.Text =
                $"{Application.TotalPaints - paintsNow} paints using {mechanism}".ToConsoleString(
                    ConsoleColor.Black,
                    ConsoleColor.Red,
                    true);
        ...
        var animationLabel = animationPanel.Add(new Label { Text = "That's all folks".ToBlack(ConsoleColor.Green) })
            .CenterBoth();

        await animationPanel.AnimateAsync(
            new ConsoleControlAnimationOptions {
                Duration = 1000,
