...
{
    private static readonly ConsoleString Space = new(" ");
    ...
    /// <param name="columnOverflowBehaviors">
    ///     Optionally provide hints as to how overflow should be handled.  By default, the
    ///     longest value in a column determines the column width.  You can choose to truncate or to do a smart wrap.
    /// </param>
    /// <param name="gutter">How many empty spaces to place between columns</param>
    /// <returns>The table, as a console string</returns>
    public ConsoleString FormatAsTable(
        List<ConsoleString> columnHeaders,
        List<List<ConsoleString>> rows,
        string? rowPrefix = "",
        List<ColumnOverflowBehavior>? columnOverflowBehaviors = null,
        int gutter = 3)
    {
        var context = new ConsoleTableBuilderContext();
        context.Gutter = gutter;
        context.ColumnHeaders = columnHeaders;
        context.Rows = rows;
        context.OverflowBehaviors =
            columnOverflowBehaviors ?? CreateDefaultOverflowBehavior(context.ColumnHeaders.Count);
    ...
    /// <param name="format">
    ///     A space delimited set of properties to use as column headers.
    ///     You can change the display string for a particular property by using the format 'PropertyName>DisplayName' where
    ///     PropertyName is a property name and DisplayName is the text to display.  If you omit this parameter then the first
    ///     object in the collection will be inspected and it's public properties will be used as columns.
    /// </param>
    /// <returns></returns>
    public ConsoleString FormatAsTable(IEnumerable objects, string? format = null)
    {
        var iter = objects.Cast<object>().ToArray();
        var prototype = GetPrototype(iter);
        ...
        format = !string.IsNullOrEmpty(format) ? format : InferFormat(prototype);
        var documentTemplate = "{{ table objects " + format + " !}}";
        var renderer = new DocumentRenderer();
        var document = renderer.Render(documentTemplate, new { objects = iter });
        return document;
    }

    private object? GetPrototype(IEnumerable objects) => objects.Cast<object?>().FirstOrDefault();
    ...
    private void DetermineColumnWidths(ConsoleTableBuilderContext context)
    {
        ...
        for (var i = 0; i < context.ColumnHeaders.Count; i++)
            ret.Add(i, context.ColumnHeaders[i].Length);

        for (var i = 0; i < context.ColumnHeaders.Count; i++)
        {
            if (context.OverflowBehaviors[i] is TruncateOverflowBehavior truncate)
            {
                ret[i] = truncate.ColumnWidth + truncate.TruncationText.Length;
            }
            else if (context.OverflowBehaviors[i] is SmartWrapOverflowBehavior smart)
            {
                if (smart.DefineMaxWidthBasedOnConsoleWidth)
                {
                    if (i != context.ColumnHeaders.Count - 1)
                    {
                        throw new InvalidOperationException(
                            "DefineMaxWidthBasedOnConsoleWidth can only be set to true for the last column in a table");
                    }
                    ...
                    smart.MaxWidthBeforeWrapping =
                        ConsoleProvider.Current.BufferWidth -
                        left -
                        context.Gutter; // The Gutter is so newlines don't get double rendered on the console
                }

                ret[i] = smart.MaxWidthBeforeWrapping;
            }
            else if (context.OverflowBehaviors[i] is GrowUnboundedOverflowBehavior)
            {
                foreach (var row in context.Rows)
                    ret[i] = Math.Max(ret[i], row[i]?.Length ?? 0);
            }
            else
            {
                throw new NotSupportedException(
                    "Unsupported overflow behavior: '" + context.OverflowBehaviors[i].GetType().FullName + "'");
            }
        }
    ...
    private void AddCellsToTable(ConsoleTableBuilderContext context)
    {
        for (var rowIndex = 0; rowIndex < context.Rows.Count; rowIndex++)
        {
            ...
            for (var colIndex = 0; colIndex < context.ColumnHeaders.Count; colIndex++)
    ...
    private void AddCellToTable(ConsoleTableBuilderContext context, int rowIndex, int colIndex)
    {
        ...
        if (context.OverflowBehaviors[colIndex] is TruncateOverflowBehavior truncate)
        {
            val = TruncateIfNeeded(truncate, val);
            while (val.Length < context.ColumnWidths[colIndex] + context.Gutter) val += Space;
        }
        else if (context.OverflowBehaviors[colIndex] is SmartWrapOverflowBehavior smart)
        {
            var segments = FormatAsWrappedSegments(smart, val);

            val = segments[0];

            if (smart.DefineMaxWidthBasedOnConsoleWidth == false)
            {
                while (val.Length < context.ColumnWidths[colIndex] + context.Gutter) val += Space;
            }

            for (var segIndex = 1; segIndex < segments.Count; segIndex++)
            {
        ...
        {
            throw new NotSupportedException(
                "Unsupported overflow behavior: '" + context.OverflowBehaviors[colIndex].GetType().FullName + "'");
        }
    ...
    private void ProcessPendingSmartWrapInterrupts(ConsoleTableBuilderContext context)
    {
        while (context.SmartWrapInterrupts.Count > 0)
        {
            ...
            for (var colIndex = 0; colIndex < context.ColumnHeaders.Count; colIndex++)
            {
                var nextInterruptCol =
                    context.SmartWrapInterrupts.Count > 0 ? context.SmartWrapInterrupts[0].Column : -1;
                ...
                else
                {
                    ...
                    if (context.OverflowBehaviors[colIndex] is SmartWrapOverflowBehavior smart && smart.DefineMaxWidthBasedOnConsoleWidth == false)
                    {
                        while (val.Length < context.ColumnWidths[colIndex] + context.Gutter) val += Space;
                    }
    ...
    private List<ConsoleString> FormatAsWrappedSegments(SmartWrapOverflowBehavior smartWrap, ConsoleString value)
    {
        ...
        while (value.Length > smartWrap.MaxWidthBeforeWrapping)
        {
            ...
            var lookBehindLimit = smartWrap.WordBreakLookBehind;
            for (var i = segment.Length - 1; i >= 0 && lookBehindLimit > 0; i--)
            {
    ...
    private ConsoleString TruncateIfNeeded(TruncateOverflowBehavior truncateBehavior, ConsoleString value)
    {
        ...
        {
            value = value.Substring(0, truncateBehavior.MaxWidthBeforeShowingTruncationText) +
                truncateBehavior.TruncationText;
        }
    ...
    private void ValidateInputs(ConsoleTableBuilderContext context)
    {
        if (context.OverflowBehaviors.Count != context.ColumnHeaders.Count)
        {
            throw new ArgumentOutOfRangeException(
                "If 'columnOverflowBehaviors' is specified then it must contain the same number of items as 'columns'");
        }
    }

    private class ConsoleTableBuilderContext
    {
        public ConsoleString RowPrefix { get; set; } = ConsoleString.Empty;
        public bool LastColumnAutoExpands { get; set; }
        public List<ConsoleString> ColumnHeaders { get; set; } = new();
        public List<List<ConsoleString>> Rows { get; set; } = new();
        public List<ColumnOverflowBehavior> OverflowBehaviors { get; set; } = new();
        public Dictionary<int, int> ColumnWidths { get; set; } = new();
        public List<ConsoleCharacter> TableOutput { get; set; } = new();
        public int Gutter { get; set; }
        public List<SmartWrapInterrupt> SmartWrapInterrupts { get; set; } = new();
    }
    ...
    private class SmartWrapInterrupt : IComparable<SmartWrapInterrupt>
    {
        ...
        public ConsoleString Value { get; set; } = ConsoleString.Empty;
        public SmartWrapOverflowBehavior SmartWrap { get; set; } = new();
...
/// <summary>
///     An abstract class that lets you describe how to handle variable column widths when formatting a console table.  You
///     should
...
/// <summary>
///     A class that indicates that the target column should be sized based on the longest value in the column, with no
///     upper bound
/// </summary>
...
public class TruncateOverflowBehavior : ColumnOverflowBehavior
{
    ...
    /// </summary>
    public int MaxWidthBeforeShowingTruncationText => ColumnWidth - TruncationText.Length;
}
...
public class SmartWrapOverflowBehavior : ColumnOverflowBehavior
{
    ...
    /// <summary>
    ///     If set to true then the target column will have its width dynamically determined based on the width of the current
    ///     console.
